...
public class NmapScannerDao {
	 ...
	 * @return
	 */
	public InternetHost getInternetHostMergedWithSaved(
			InternetHost newInternetHost) {
		InternetHost savedInternetHost = retrieveHostByIp(newInternetHost
				.getIp());
		if (savedInternetHost != null) {
			...
			for (HostAlias savedAlias : savedInternetHost.getHostAliases()) {
				...
				for (HostAlias newAlias : newInternetHost.getHostAliases()) {
					if (savedAlias.getFqdn() != null
							&& savedAlias.getFqdn().equals(newAlias.getFqdn())) {
	...
	public InternetHost retrieveHostByFqdn(String hostname) {
		...
		Transaction tx = session.beginTransaction();
		Query query = session
				.createQuery("from HostAlias as alias inner join alias.internetHost as host where alias.fqdn=:fqdn");
	...
	public InternetHost retrieveHostByIp(String ip) {
		...
		Transaction tx = session.beginTransaction();
		Query query = session
				.createQuery("from InternetHost as host where host.ip=:ip");
	...
	public HostAlias retrieveHostAliasByFqdn(String fqdn) {
		...
		Transaction tx = session.beginTransaction();
		Query query = session
				.createQuery("from HostAlias as alias where alias.fqdn=:fqdn");
	...
	public List<NmapScan> retrieveNmapScansByHost(String host) {
		...
		Transaction tx = session.beginTransaction();
		Query query = session
				.createQuery("from NmapScan as scan where scan.internetHost.ip=:ip");
