...
public class Scan {
	...
	@Produces(MediaType.APPLICATION_JSON)
	  ...
	  public List<SiteScan> scanHost(@PathParam("host") String host) {
		...
		System.out.println("Scan $"+cc++);
//		List<NmapScan> nmapScans=NmapScannerDao.getInstance().retrieveNmapScansByHost(host);
        ObjectMapper objectMapper = new ObjectMapper();
        try {
    		System.out.println("Scan @"+cc++);
			String json=objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(scans);
			System.out.println("Scan "+cc++);
