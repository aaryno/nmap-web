 ...
 * src/main/resources/nmapweb.properties. Output is generated as xml using the
 * -oX option. Ports are 0-1000 by default but can accept an arbitrary list
 * (e.g., 1-5,80-90,95).
 * 
 * @author aaryno1
 *
...
public class NmapScanner {
	 ...
	 * @throws IOException
	 */
	public static NmapScan scan(String host, String portRange)
			throws IOException {
		List<Integer> ports = getPortsFromPortRange(portRange);
		String outputFile = "nmap_out." + UUID.randomUUID().toString();
		try {
			String nmapPath = NmapWebProperties.getInstance().getProperties()
					.getProperty("nmap_path");
			String[] cmdArray = new String[] { nmapPath, "-oX", outputFile,
					"-v", host, "-p", portRange };
			...
			nmapScan.setScanDate(new java.sql.Timestamp(Calendar.getInstance()
					.getTime().getTime()));
			return nmapScan;
		} finally {
	 ...
	 * 
	 * @param nmapOutputFile
	 *            name of output file produced by nmap, notably the [file]
	 *            specified by -oX [file]
	 * 
	 *            Caveat to the reader: a little hairy with the nested loops as
	 *            we traverse the DOM plus it has DAO dependency built in to
	 *            consolidate object reconciliation (i.e., don't make a new
	 *            InternetHost if there's already open in the database). I
	 *            prefer to separate these functions.
	 * 
	 * @return NmapScan, which is a hibernate entity This will be fully
	 *         populated with the ScanPorts (and their state),
	 * @throws NmapException
	 */
	public static NmapScan readScanOutput(String nmapOutputFile,
			List<Integer> ports) throws NmapException {
		NmapScan nmapScan = new NmapScan();
		...
		if (nmaprunElement != null) {
			...
			if (hostElement == null) {
				...
				nmapScan.setScanDate(new java.sql.Timestamp(Calendar
						.getInstance().getTimeInMillis()));
				InternetHost internetHost = null;
				if (addressElement != null) {
					String ip = addressElement.getAttributeValue("addr");
					String ipType = addressElement
							.getAttributeValue("addrtype");
					internetHost = NmapScannerDao.getInstance()
							.retrieveHostByIp(ip);
				...
				if (internetHost == null) {
					throw new NmapException(
							"Parse error, unable to find host in doc");
				}

				Element hostnamesElement = hostElement.getChild("hostnames");
				if (hostnamesElement != null) {
					List<Element> hostnameList = hostnamesElement
							.getChildren("hostname");
					for (Element hostnameElement : hostnameList) {
						String hostname = hostnameElement
								.getAttributeValue("name");
						if (hostname != null) {
							boolean found = false;
							if (internetHost.getHostAliases() == null) {
								internetHost
										.setHostAliases(new HashSet<HostAlias>());
							}
							for (HostAlias hostAlias : internetHost
									.getHostAliases()) {
								if (hostAlias.getFqdn() != null
										&& hostAlias.getFqdn().equals(hostname)) {
									found = true;
								}
							}
							if (!found) {
								HostAlias hostAlias = NmapScannerDao
										.getInstance().retrieveHostAliasByFqdn(
												hostname);
								if (hostAlias == null) {
								...
								System.out.println("hostalias fqdn: "
										+ hostAlias.getFqdn());
							}
				...
				if (portsElement != null) {
					...
					for (int i = 0; i < portsList.size(); i++) {
						...
						if (portElement != null) {
							String portid = portElement
									.getAttributeValue("portid");
							Integer port = null;
							...
							Element stateElement = portElement
									.getChild("state");
							if (stateElement != null) {
								String state = stateElement
								...
								throw new NmapException(
										"Parse error, unable to find port state in doc");
							}
						} // end portElement
						else {
							throw new NmapException(
									"Parse error, unable to find port element in doc");
						}
					} // end portsList loop
				} // end portsElement
				else {
					throw new NmapException(
							"Parse error, unable to find ports in doc");
				}
			}
		}
		return nmapScan;
