...
/**
 * Factory class to generating SiteScan and SiteScanHistory objects from
 * NmapScans
 * 
 * @author aaryno1
 *
...
public class SiteScanFactory {
	...
	public static SiteScan buildSiteScan(NmapScan nmapScan) {
		...
		try {
			...
			if (nmapScan.getInternetHost().getHostAliases() != null) {
				Iterator<HostAlias> iterator = nmapScan.getInternetHost()
						.getHostAliases().iterator();
	...
	public static SiteScanHistory buildSiteScanHistory(List<NmapScan> nmapScans) {
		...
		// make a master list of ports in case previous scans used different
		// ports
		...
		String[][] portStateArray = new String[nmapScans.size()][orderedPorts
				.size()];
		int[][] portStateChanges = new int[nmapScans.size()][orderedPorts
				.size()];
		...
		for (NmapScan nmapScan : nmapScans) {
			for (ScanPort scanPort : nmapScan.getScanPorts()) {
				...
				if (lastStateStr != null
						&& !lastStateStr.equals(scanPort.getState())) {
					portStateChanges[scan][port] = getValueChange(
							scanPort.getState(), lastStateStr);
				}
